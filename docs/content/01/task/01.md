...
Check in 👍 at [this issue](https://github.com/digital-sustainability/module-eoss/issues/29)
///
```
gh issue view --json comments https://github.com/digital-sustainability/module-eoss/issues/29 -q ".[].[] | .author.login" | sort -u --ignore-case > t01-01.txt
```

```
rm -f t01-01-comments.txt && for login in $(cat logins.txt); do echo $login && gh issue view --json comments https://github.com/digital-sustainability/module-eoss/issues/29 -q ".[].[] | select(.author.login == \"$login\") | .body" && echo;done >> t01-01-comments.txt
```